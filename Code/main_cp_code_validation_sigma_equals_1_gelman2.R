...
set.seed(5)
delta_temp = sort(runif(no_of_categ -1, -20, 20))
...
while (i > 0){
  ...
  mcmc_obj_list_training[[i]] = cbind(c(beta_mat_preconvergence_training[,1],beta_star_mat_preconvergence_training[,1], 
                                        t0_preconvergence_training[1]), 
                                      c(beta_mat_preconvergence_training[,2],beta_star_mat_preconvergence_training[,2],
                                        t0_preconvergence_training[2]), 
                                      c(beta_mat_preconvergence_training[,3],beta_star_mat_preconvergence_training[,3],
                                        t0_preconvergence_training[3]))
  ...
  if( i %% 50 == 0){
    ...
    for (gl in 1:(tot_gel_var)){
      plist = lapply(apply(do.call(rbind, lapply(mcmc_obj_list_training,function(x) x[gl,])),2,as.data.frame),
                     mcmc)
  ...
  if(converged){
    ...
    if(length(sigma1_sq_sample_training) == sample_size){
      break
    }
