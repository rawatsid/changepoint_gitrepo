...
for(rho_tmp in rho_vec_tmp_training){
  for (rho_sp in rho_vec_sp_training) {
    ...
    for (i in (1:(burn_period_training + sample_size_training*diff_in_random_draws_training))) {
      ...
      for (st in 1:n_total_training) {
        ...
        if(st_week <= changepoint_t_0_week){
          pi_st = truncnorm::rtruncnorm(1, a = delta_training[y_val_st], b = delta_training[y_val_st + 1],
                                        mean = as.numeric(t(x_vec_st) %*% beta_training + v_val_st), 
                                        sd = sqrt(sigma2_sq_training))
        }else{
          pi_st = truncnorm::rtruncnorm(1, a = delta_training[y_val_st], b = delta_training[y_val_st + 1],
                                        mean = as.numeric(t(x_vec_st) %*% beta_star_training + v_val_st), 
      ...
      X_mat_pre_changepoint_training = matrix(rep(1,n_sp_training * n_tmp_pre_changepoint_training), ncol = p+1)
      
      X_mat_post_changepoint_training = matrix(rep(1,n_sp_training * n_tmp_post_changepoint_training), ncol = p+1)
      ...
      for (tp in (changepoint_vec)) {
        ...
        X_mat_pre_t = matrix(rep(1,n_sp_training * tp), ncol = 1)
        
        X_mat_post_t = matrix(rep(1,n_sp_training * (n_tmp_training - tp)), ncol = 1)
        ...
        log_prop_pmf_t_0_training = ((-n_sp_training*tp/2)*log(2*pi*sigma2_sq_training) + 
                                       ...
                                       ((-1/(2*sigma1_sq_star_training)) * 
                                          (t(v_vec_post_t) %*% 
                                             inv_SIGMA_sp_kro_inv_SIGMA_tmp_post_t_mat_list[[tp/gap_bw_changepoints - 1]] %*% 
                                             v_vec_post_t)))
