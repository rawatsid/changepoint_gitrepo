...
if (colnames(df1)[ncol(df1)] == lastcol){
  tmp = unlist(strsplit(substr(lastcol,2,nchar(lastcol)),"[.]"))
  tmp[3] = paste(c("20",tmp[3]),collapse = "")
  string = paste(tmp,collapse = ".")
...
setwd("~/OneDrive - Indian Institute of Management/PhD Students/Siddharth - PhD work/Spacetime changepoint/R/")
write.csv(x = usdata,
          file = "US_COVID_data.csv",row.names = F)



#:::: RUN IF NECESSARY (PREVIOUS CODE TO MAKE STATEWISE WEEKLY DATA)
