...
y_training = training_df_clone$pi_vec_art
...
sigma2_sq_preconvergence_training = c(0.5, 0.75, 1.5)

sigma2_sq_star_preconvergence_training = c(0.5, 0.75, 1.5)

t0_preconvergence_training = rep(t0_art, 3)  #c(changepoint_vec[floor(length(changepoint_vec)/2)],
...
# set.seed(5)
# delta_temp = sort(runif(no_of_categ -1, 13, 156))#taking values from min and max pi vec art
# delta_mat_preconvergence_training = cbind(delta_temp, sort(delta_temp + rnorm(3)),
#                                           sort(delta_temp + rnorm(3)))

# delta_mat_preconvergence_training = cbind(delta_art, delta_art, delta_art)# rbind(c(-Inf, -Inf, -Inf),delta_mat_preconvergence_training,#
#c(Inf, Inf, Inf))
...
while (i > 0){
  ...
  if(!converged){
    ...
    for (mc in (1:3)) {
      ...
      #beta posterior###################################
      st_beta = Sys.time()
      pi_pre_changepoint_training = training_pre_changepoint_df$pi_vec_art#[[pi_vec_mc_col]]#pi_mat_pre_changepoint_training[1:(n_sp_training * n_tmp_pre_changepoint_training)]
      pi_post_changepoint_training = training_post_changepoint_df$pi_vec_art#[[pi_vec_mc_col]]#pi_mat_pre_changepoint_training[1:(n_sp_training * n_tmp_post_changepoint_training)]
      ...
      beta_mean_training = beta_covar_training %*% ( t(X_mat_pre_changepoint_training)%*% 
                                                       (pi_pre_changepoint_training))/sigma2_sq_training # - v_training
      ...
      temp_vec2 = pi_post_changepoint_training - X_mat_post_changepoint_training %*% beta_star_training #- v_star_training
      ...
      for (tp in (changepoint_vec)) {
        ...
        X_mat_pre_t = cbind(rep(1,n_sp_training * tp),
                            log(training_pre_t_df$population),
                            training_pre_t_df$time,
                            training_pre_t_df$time_sq,
                            training_pre_t_df$prev_log_new_death)

        X_mat_post_t = cbind(rep(1,n_sp_training * (n_tmp_training - tp)),
                             log(training_post_t_df$population),
                             training_post_t_df$time,
                             training_post_t_df$time_sq,
                             training_post_t_df$prev_log_new_death)

        # v_vec_pre_t = training_pre_t_df [[v_vec_mc_col]]
        # v_vec_post_t = training_post_t_df [[v_vec_mc_col]]
        pi_vec_pre_t = training_pre_t_df$pi_vec_art #[[pi_vec_mc_col]]
        pi_vec_post_t = training_post_t_df$pi_vec_art #[[pi_vec_mc_col]]
        ...
        log_prop_pmf_t_0_training = ((-n_sp_training*tp/2)*log(2*pi*sigma2_sq_training) +
                                       ...
                                       ((-n_sp_training*(n_tmp_post_t)/2) * log(2*pi*sigma2_sq_star_training)) +
  ...
  if(converged){
    ...
    #beta posterior###################################
    st_beta = Sys.time()
    pi_pre_changepoint_training = (training_df_clone %>% filter(week <= changepoint_t_0_week))$pi_vec_art#pi_vec#pi_mat_pre_changepoint_training[1:(n_sp_training * n_tmp_pre_changepoint_training)]
    pi_post_changepoint_training = (training_df_clone %>% filter(week > changepoint_t_0_week))$pi_vec_art#pi_vec#pi_mat_pre_changepoint_training[1:(n_sp_training * n_tmp_post_changepoint_training)]
    # v_training = (training_df_clone %>% filter(week <= changepoint_t_0_week))[[v_vec_mc_col]]#v_mat_training[1:(n_sp_training * n_tmp_pre_changepoint_training)]
    # v_star_training = (training_df_clone %>% filter(week > changepoint_t_0_week))[[v_vec_mc_col]] #v_star_mat_training[1:(n_sp_training * n_tmp_post_changepoint_training)]
    #dividing by 10^4 for large variance for beta prior
    beta_covar_training = solve( (t(X_mat_pre_changepoint_training) %*% 
    ...
    beta_star_mean_training = (beta_star_covar_training %*% ( t(X_mat_post_changepoint_training)%*% 
                                                                (pi_post_changepoint_training))/sigma2_sq_star_training) # - v_star_training
    ...
    for (tp in (changepoint_vec)) {
      ...
      pi_vec_pre_t = (training_df_clone %>% filter(week <= week_t))$pi_vec_art
      pi_vec_post_t = (training_df_clone %>% filter(week > week_t))$pi_vec_art
    ...
    if(length(sigma2_sq_sample_training) == sample_size_training){
      break
    }
...
sum(avg_tt_v,avg_tt_beta, avg_tt_sigma1_sq, avg_tt_sigma2_sq, avg_tt_delta, 
    avg_tt_pi, avg_tt_t_0)
