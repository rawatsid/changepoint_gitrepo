...
if(is_all_loc){
  training_df = df %>% filter(week <= validation_wk_min)
...
n_total_training = length(y_training)
...
v_vec_mean_training = vector()
pi_vec_mean_training = vector()
delta_mean_training = vector()
beta_sample_mean_training = vector()
beta_star_sample_mean_training = vector()
sigma1_sq_sample_mean_training = vector()
sigma1_sq_star_sample_mean_training = vector()
sigma2_sq_sample_mean_training = vector()
sigma2_sq_star_sample_mean_training = vector()
beta_significance_mat_training = vector()
beta_star_significance_mat_training = vector()
t_0_mean_training = vector()
...
rho_vec_tmp_training = c(0.10, 0.50, 0.75, 1, 1.5, 3)#seq(0.1, 0.5, 0.1)
rho_vec_sp_training = c(0.006, 0.01, 0.05, 0.1, 0.25, 0.5)#seq(0.5, 1, 0.1) 0.0001, 0.001,, 0.5
rho_idx = 1

gap_bw_changepoints = 5
changepoint_vec = seq(gap_bw_changepoints, n_tmp_training- gap_bw_changepoints, by = gap_bw_changepoints)# - gap_bw_changepoints
#Getting y in matrix form
#y_mat_training = matrix(y_training,nrow = n_tmp_training,ncol = n_sp_training)


for(rho_tmp in rho_vec_tmp_training){
  for (rho_sp in rho_vec_sp_training) {
    ...
    training_df_clone$v_vec = rep(0,n_total_training)
    v_mat_training = matrix(((training_df_clone %>% filter(week <= changepoint_t_0_week))$v_vec),
                            nrow = n_tmp_pre_changepoint_training, ncol = n_sp_training)
    
    v_star_mat_training = matrix(((training_df_clone %>% 
                                     filter(week > changepoint_t_0_week))$v_vec),
                                 nrow = (n_tmp_post_changepoint_training), ncol = n_sp_training)
    
    beta_training = rep(0,p+1)
    beta_star_training = rep(0,p+1)
    ...
    #Check if need to do it first by spatial location and then time points can be done
    ...
    #Saving  Sigma_v matrix for different t_0
    ...
    for (tp in (changepoint_vec)) {
      ...
      inv_SIGMA_tmp_pre_t_mat_list[[tp/gap_bw_changepoints]] = inv_SIGMA_tmp_pre_t
      inv_SIGMA_tmp_post_t_mat_list[[tp/gap_bw_changepoints]] = inv_SIGMA_tmp_post_t
      
      inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_t = (inv_SIGMA_sp_training %x% inv_SIGMA_tmp_pre_t)
      inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_t_mat_list[[tp/gap_bw_changepoints]] = inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_t
      ...
      inv_SIGMA_sp_kro_inv_SIGMA_tmp_post_t_mat_list[[tp/gap_bw_changepoints]] = inv_SIGMA_sp_kro_inv_SIGMA_tmp_post_t
      log_det_SIGMA_sp_kro_inv_SIGMA_tmp_post_t = (tp * det_log_SIGMA_sp_training + 
    ...
    for (i in (1:(burn_period_training + sample_size_training*diff_in_random_draws_training))) {
      ...
      for (sp in (1:n_sp_training)){
        ...
        inv_SIGMA_cond_pre_changepoint_training = (inv_SIGMA_tmp_pre_t_mat_list[[t_0_training/gap_bw_changepoints]]/
                                                     mat_mult_temp)
        
        inv_SIGMA_cond_post_changepoint_training = (inv_SIGMA_tmp_post_t_mat_list[[t_0_training/gap_bw_changepoints]]/
                                                      mat_mult_temp)
      ...
      sigma1_sq_lambda_training = (t(v_training) %*% 
                                     inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_t_mat_list[[t_0_training/gap_bw_changepoints]] 
                                   %*% v_training)/2 + lambda_training
      
      sigma1_sq_star_lambda_training = (t(v_star_training) %*% 
                                          inv_SIGMA_sp_kro_inv_SIGMA_tmp_post_t_mat_list[[t_0_training/gap_bw_changepoints]] 
                                        %*% v_star_training)/2 + lambda_training
      
      sigma1_sq_training = invgamma::rinvgamma(1,sigma1_sq_a_training,rate = sigma1_sq_lambda_training)
      
      sigma1_sq_star_training = invgamma::rinvgamma(1, sigma1_sq_star_a_training,
      ...
      sigma2_sq_training = invgamma::rinvgamma(1,sigma2_sq_a_training,rate = sigma2_sq_lambda_training)
      
      sigma2_sq_star_training = invgamma::rinvgamma(1, sigma2_sq_star_a_training,
      ...
      for (tp in (changepoint_vec)) {
        ...
        log_prop_pmf_t_0_training = ((-n_sp_training*tp/2)*log(2*pi*sigma2_sq_training) + 
                                   ...
                                   ((-1/(2 * sigma1_sq_training)) * 
                                      (t(v_vec_pre_t) %*% 
                                         inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_t_mat_list[[tp/gap_bw_changepoints]] 
                                       %*% v_vec_pre_t)) + 
                                   ((-n_sp_training*(n_tmp_training - tp)/2) * 
                                   ...
                                   ((-1/(2*sigma1_sq_star_training)) * 
                                      (t(v_vec_post_t) %*% 
                                         inv_SIGMA_sp_kro_inv_SIGMA_tmp_post_t_mat_list[[tp/gap_bw_changepoints]] %*% 
                                         v_vec_post_t)))
    ...
    training_df_clone = training_df_clone %>% 
      mutate(pred_category = ifelse(pi_vec_est > mean_delta_training[3],4,
                                    ifelse(pi_vec_est > mean_delta_training[2], 3,
                                           ifelse(pi_vec_est > mean_delta_training[1],2,1))))
    ...
    beta_star_significance_mat_training = cbind(beta_star_significance_mat_training, beta_star_significance_vec)
    
    #Saving samples for each rho
    v_mat_sample_list_training[[rho_idx]] = v_sample_training
    ...
    v_sample_mean_training = cbind(v_sample_mean_training, rowMeans(v_sample_training))
    beta_sample_mean_training = cbind(beta_sample_mean_training, rowMeans(beta_sample_training))
    sigma1_sq_sample_mean_training = c(sigma1_sq_sample_mean_training, mean(sigma1_sq_sample_training))
    sigma2_sq_sample_mean_training = c(sigma2_sq_sample_mean_training, mean(sigma2_sq_sample_training))
    
    #Validation
    #v_pred_val_vec = vector()
    #y_pred_val_vec = vector()
    y_pred_val_mat = vector()
    pop_vec_validation = unique(validation_df[,c(2,6)])[,2]
    for (pred_i in (1:n_total_validation)) {
      Sigma_12_vec = vector()
      for (sp in (1:n_total_training)) {
        sp_dist = distm(c(training_df$long[sp],  training_df$lat[sp]), c(validation_df$long[pred_i], validation_df$lat[pred_i]), fun = distHaversine)
        tmp_dist = abs((as.numeric(difftime(validation_df$week[pred_i] ,  training_df$week[sp]), units="days"))/7)
        Sigma_12_vec = c(Sigma_12_vec, exp(-rho_sp * sp_dist/1000) * exp(-rho_tmp * tmp_dist))
      }
      v_tmp_given_v1_covar = (1 - t(Sigma_12_vec) %*% inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_changepoint_training %*%
                                Sigma_12_vec)
      v_tmp_given_v1_mean = (t(Sigma_12_vec) %*% inv_SIGMA_sp_kro_inv_SIGMA_tmp_pre_changepoint_training %*% 
                               rowMeans(v_sample_training))
      #Get 1000 values from the posterior distribution
      v_tmp_given_v1 = (v_tmp_given_v1_mean + sqrt(mean(sigma1_sq_sample_training)) *
                          t(chol(v_tmp_given_v1_covar)) * rnorm(1000))
      
      X_vec_tmp = c(1, log(validation_df$population[pred_i]), validation_df$time[pred_i], validation_df$time_sq[pred_i], 
                    validation_df$prev_log_new_death[pred_i])
      #drawing 1000 samples
      y_pred_val = t(X_vec_tmp) %*% rowMeans(beta_sample_training) + v_tmp_given_v1 + 
        sqrt(mean(sigma2_sq_sample_training))*rnorm(1000)
      #y_pred_val_vec = c(y_pred_val_vec, y_pred_val)
      #v_pred_val_vec = c(v_pred_val_vec, v_tmp_given_v1)
      y_pred_val_mat = rbind(y_pred_val_mat,y_pred_val)
    }
    #Saving Y pred validation matrix
    y_pred_val_mat_list_training[[rho_idx]] =  y_pred_val_mat
    
    mape_validation = (sum(abs( (y_validation - rowMeans(y_pred_val_mat))/y_validation )))/n_total_validation
    mape_vec_validation_training = c(mape_vec_validation_training, mape_validation)
