...
sigma1_sq_art = 1
sigma1_sq_star_art = 2
sigma2_sq_art = 1
sigma2_sq_star_art = 2.5
...
X_mat_pre_t0_art = cbind(rep(1,n_sp_training * t0_art), training_pre_t0_art_df$X1, 
                         training_pre_t0_art_df$X2, training_pre_t0_art_df$X3, 
                         training_pre_t0_art_df$X4)

X_mat_post_t0_art = cbind(rep(1,n_sp_training * (n_tmp_training - t0_art)), 
                          training_post_t0_art_df$X1, training_post_t0_art_df$X2, 
                          training_post_t0_art_df$X3, training_post_t0_art_df$X4)
...
training_pre_t0_art_df$v_vec_art = as.numeric(v_vec_art)

training_post_t0_art_df$v_vec_art = as.numeric(v_vec_star_art)

training_df_clone = (rbind(training_pre_t0_art_df, training_post_t0_art_df) %>% 
                       arrange(country, state,key, lat, long, id,  week))

delta_art = c(-Inf, -6, 3.5, 13, Inf)
...
lambda_training = 1
stdnorm = 100
alpha_vec = rep(1, no_of_categ)#rep(1,no_of_categ)
...
training_df_clone$v_vec1 = training_df_clone$v_vec_art#rep(0, n_total_training)#
training_df_clone$v_vec2 = training_df_clone$v_vec_art#rep(0, n_total_training)
training_df_clone$v_vec3 = training_df_clone$v_vec_art#rep(0, n_total_training)

# training_df_clone$pi_vec1 = training_df_clone$pi_vec_art #rep(0, n_total_training)#
# training_df_clone$pi_vec2 = training_df_clone$pi_vec_art
# training_df_clone$pi_vec3 = training_df_clone$pi_vec_art
...
sigma1_sq_preconvergence_training = c(1, 1.5, 2)

sigma1_sq_star_preconvergence_training = c(1, 1.5, 2)

sigma2_sq_preconvergence_training = c(1, 1.5, 2)

sigma2_sq_star_preconvergence_training = c(1, 1.5, 2)

t0_preconvergence_training = c(changepoint_vec[floor(length(changepoint_vec)/2)], #  rep(t0_art, 3)
...
set.seed(5)
delta_temp = sort(runif(no_of_categ -1, -20, 20))#taking values from min and max pi vec art
delta_mat_preconvergence_training = cbind(delta_temp, sort(delta_temp + rnorm(3)),
                                          sort(delta_temp + rnorm(3)))
...
while (i > 0){
  ...
  if(!converged){
    ...
    for (mc in (1:3)) {
      ...
      sigma1_sq_training = sigma1_sq_preconvergence_training[mc]
      sigma1_sq_star_training = sigma1_sq_star_preconvergence_training[mc]
      sigma2_sq_training = sigma2_sq_preconvergence_training[mc]
      sigma2_sq_star_training = sigma2_sq_star_preconvergence_training[mc]
      ...
      for (st in 1:n_total_training) {
        ...
        x_vec_st = c(1, training_df_clone$X1[st], training_df_clone$X2[st],
                     training_df_clone$X3[st], training_df_clone$X4[st])
      ...
      X_mat_pre_changepoint_training = cbind(rep(1,n_sp_training * n_tmp_pre_changepoint_training), 
                                             training_pre_changepoint_df$X1, 
                                             training_pre_changepoint_df$X2, 
                                             training_pre_changepoint_df$X3, 
                                             training_pre_changepoint_df$X4)
      
      X_mat_post_changepoint_training = cbind(rep(1,n_sp_training * n_tmp_post_changepoint_training), 
                                              training_post_changepoint_df$X1, 
                                              training_post_changepoint_df$X2, 
                                              training_post_changepoint_df$X3, 
                                              training_post_changepoint_df$X4)
      ...
      #                                          nrow = n_tmp_pre_changepoint_training, 
      #                                          ncol = n_sp_training, byrow = FALSE)
      # 
      # pi_mat_post_changepoint_training = matrix((training_post_changepoint_df[[pi_vec_mc_col]]),
      #                                           nrow = (n_tmp_post_changepoint_training), 
      #                                           ncol = n_sp_training, byrow = FALSE)
      # 
      # 
      # 
      ...
      #                        diag(n_sp_training)[idx,])[(1+n_tmp_pre_changepoint_training):
      #                                                     (n_tmp_pre_changepoint_training *
      #                                                        n_sp_training)]
      #   
      #   v_cond_star_training = (v_star_mat_training %*% 
      #                             diag(n_sp_training)[idx,])[(1+n_tmp_post_changepoint_training):
      ...
      #   
      #   v_star_covar_training = chol2inv(chol(inv_SIGMA_cond_post_changepoint_training/
      #                                           sigma1_sq_star_training +
      #                                           diag(n_tmp_post_changepoint_training)/
      ...
      #   
      # }
      # training_pre_changepoint_df[[v_vec_mc_col]] = v_mat_training[1:(n_sp_training * n_tmp_pre_changepoint_training)]
      # training_post_changepoint_df[[v_vec_mc_col]] = v_star_mat_training[1:(n_sp_training * n_tmp_post_changepoint_training)]
      # 
      ...
      v_training = (training_df_clone %>% filter(week <= changepoint_t_0_week))[[v_vec_mc_col]]   #v_mat_training[1:(n_sp_training * n_tmp_pre_changepoint_training)]
      v_star_training = (training_df_clone %>% filter(week > changepoint_t_0_week))[[v_vec_mc_col]]#v_star_mat_training[1:(n_sp_training * n_tmp_post_changepoint_training)]
      ...
      sigma1_sq_preconvergence_training[mc] = sigma1_sq_training
      sigma1_sq_star_preconvergence_training[mc] = sigma1_sq_star_training
      ...
      sigma2_sq_preconvergence_training[mc] = sigma2_sq_training
      sigma2_sq_star_preconvergence_training[mc] = sigma2_sq_star_training
      ...
      for (j in 1:(no_of_categ -1)){
        pi_cat1 = pi_vec_training[y_training == j] #because j starts from 1 hence category values are okay but delta vec values are shifted
        c1 = max(pi_cat1)
        pi_cat2 = pi_vec_training[y_training == (j + 1)]
        c2 = min(pi_cat2)
      ...
      for (tp in (changepoint_vec)) {
        ...
        X_mat_pre_t = cbind(rep(1,n_sp_training * tp),
                            training_pre_t_df$X1,
                            training_pre_t_df$X2,
                            training_pre_t_df$X3,
                            training_pre_t_df$X4)

        X_mat_post_t = cbind(rep(1, n_sp_training * (n_tmp_training - tp)),
                             training_post_t_df$X1,
                             training_post_t_df$X2,
                             training_post_t_df$X3,
                             training_post_t_df$X4)

        v_vec_pre_t = training_pre_t_df [[v_vec_mc_col]]
    ...
    if( i %% 50 == 0){
      ...
      if(length(which(gelmanrubinstat > 1.2)) == 0){
        ...
        sigma1_sq_training = sigma1_sq_preconvergence_training[1]
        sigma1_sq_star_training = sigma1_sq_star_preconvergence_training[1]
        sigma2_sq_training = sigma2_sq_preconvergence_training[1]
        sigma2_sq_star_training = sigma2_sq_star_preconvergence_training[1]
  ...
  if(converged){
    ...
    for (tp in (changepoint_vec)) {
      ...
      v_vec_pre_t = training_pre_t_df$v_vec
    ...
    if(length(sigma1_sq_sample_training) == sample_size_training){
      break
    }
...
avg_tt_sigma1_sq = total_tt_sigma1_sq/(i)
avg_tt_sigma2_sq = total_tt_sigma2_sq/(i)
